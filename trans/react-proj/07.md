...
如果您正在阅读本书，这意味着您已经阅读了本书的最后一部分，该部分使用 React 构建 web 应用。在前面的章节中，您已经使用了 React 的核心功能，例如呈现组件、使用上下文的状态管理和挂钩。您已经学习了如何创建 PWA 和 SSR 应用，以及如何向 React 应用添加路由。此外，您还知道如何使用 Jest 和Enzyme将测试添加到 React 应用中。让我们将 GraphQL 添加到您迄今所了解的内容列表中。
