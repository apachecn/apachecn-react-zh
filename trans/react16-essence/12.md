...
在传统意义上，Redux 不是 Flux。Flux 有一个规范和一个实现它的库。Redux不是这个。如前一节所述，Redux 是Flux的简化。它保留了所有导致良好应用体系结构的流量概念，同时忽略了可能使流量难以实现并最终难以采用的繁琐内容。
...
到目前为止，我们有处理创建新应用状态的reducer 函数，以及触发 reducer 函数的 action creator 函数。我们仍然需要将 React 组件连接到 Redux 商店。在本节中，您将学习如何使用`connect()`函数创建连接到 Redux 应用商店的组件的新版本。
