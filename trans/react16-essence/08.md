...
`Header`组件呈现`"Collection name:"`字符串。`<input>`元素被包装在`<div>`元素中，其`className`属性设置为`form-group`。这个名称是我们在[第 5 章](05.html "Chapter 5. Making Your React Components Reactive")中讨论的引导框架的一部分，*使您的React组件反应*。它用于布局和样式设置，而不是 React 应用逻辑的一部分。
