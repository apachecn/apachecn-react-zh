...
顾名思义，`fromJS()`函数用于构造不可变的数据结构。`Immutable.js`具有非常有用的功能，用于操纵React 组件的状态。我们将在本书的其余部分使用`Immutable.js`，从这个例子开始，您将了解更多的细节。
...
本章是关于避免单片组件设计的。然而，在任何React组件的设计中，整块石通常是一个必要的起点。
