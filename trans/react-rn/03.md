...
**状态**是React 组件的动态部分。这意味着您可以声明组件的初始状态，该状态会随时间而变化。
...
一个功能性组件就是它听起来的样子——一个功能。想象一下你所看到的任何React组件的`render()`方法。这种方法本质上就是组件。功能性 React 组件的工作是返回 JSX，就像基于类的 React 组件一样。不同之处在于，这是功能组件所能做的一切。它没有状态，也没有生命周期方法。
...
在本章的最后一节中，我们将介绍**容器组件**的概念。这是一种常见的React模式，它将您所了解的有关状态和属性的许多概念结合在一起。
