...
这就结束了本书的第二部分。您已经了解了如何为 Web 构建 React 组件，以及如何为移动平台构建 React 组件。在本书的开头，我假设 React 的美妙之处在于渲染目标的概念。React 的声明性编程接口永远不必更改。翻译 JSX 元素的底层机制在理论上是完全可替换的，我们可以将React渲染为任何东西。
