...
在上一章中，我们学习了 Redux 以及如何将 Redux 与挂钩结合使用。我们还学习了如何将现有的 Redux 应用程序迁移到基于钩子的解决方案。此外，我们还了解了使用 Reducer 挂钩和 Redux 挂钩的利弊，以及何时使用其中任何一种挂钩。

在本章中，我们将学习如何结合使用 MobX 和 hook。我们将从学习如何使用 MobX 处理状态开始，然后继续使用带有挂钩的 MobX。此外，我们将学习如何将现有的 MobX 应用程序迁移到 hook。最后，我们将讨论使用 MobX 的利弊。在本章结束时，您将完全了解如何使用钩子编写 MobX 应用程序。
...
MobX 采用了与 Redux 不同的方法。它的目标不是应用限制使状态更改可预测，而是自动更新从应用程序状态派生的任何内容。在 MobX 中，我们可以直接修改 state 对象，而不是分派操作，MobX 将负责更新使用状态的任何内容。
...
4.  反应类似于计算值，但它们也会产生副作用而不是值，例如在 React 中更新用户界面：
...
![](Images/980b2234-8ff9-4bb6-8d22-4447522b692a.png)

Visualization of the MobX life cycle

MobX 和 React 配合得非常好。每当 MobX 检测到状态已更改时，它将导致相应组件的重新渲染。

与 Redux 不同，使用 MobX 不需要了解很多限制。我们只需要了解一些核心概念，如可观测值、计算值和反应。

现在我们已经了解了 MobX 的生命周期，让我们继续在实践中使用 MobX 处理状态。

# 使用 MobX 处理状态

了解 MobX 的最佳方法是在实践中使用它，并了解它是如何工作的。那么，让我们首先将我们的 ToDo 应用程序从[第 11 章](11.html)移植到 MobX，将*从 React 类组件*移植到 MobX。我们从复制`Chapter11/chapter11_2/`中的代码示例开始。
...
我们现在可以在`App`组件中初始化存储，并将其传递给所有其他组件。但是，最好使用 React 上下文。这样，我们就可以从应用程序中的任何位置访问商店。MobX React 提供了一个`Provider`组件，它在上下文中提供存储。
...
我们将首先连接`App`组件，当应用程序初始化时，我们将使用`fetch`操作从 API 获取所有`todos`。
...
现在，我们的应用程序将列出所有待办事项。但是，我们还不能切换或删除待办事项。
...
在上一节中，我们学习了如何将 MobX 与 React 一起使用。正如我们所看到的，为了能够将我们的组件连接到 MobX 存储，我们需要使用`inject`函数包装它们，在某些情况下，还需要使用`observer`函数包装它们。除了使用这些高阶组件包装我们的组件之外，自从`mobx-react`版本的 v6 发布以来，我们还可以使用钩子将我们的组件连接到 MobX 商店。我们现在将使用带有挂钩的 MobX！

# 定义存储钩子

首先，我们必须定义一个钩子才能访问我们自己的商店。正如我们之前所了解的，MobX 使用 React 上下文向各种组件提供并注入状态。我们可以从`mobx-react`获取`MobXProviderContext`并创建我们自己的自定义上下文挂钩，以便访问所有存储。然后，我们可以创建另一个钩子，专门访问我们的`TodoStore`。

那么，让我们开始定义一个商店挂钩：

1.  创建一个新的`src/hooks.js`文件。
2.  从`react`导入`useContext`钩子，从`mobx-react`导入`MobXProviderContext`：
...
3.  现在，我们定义并导出一个`useStores`钩子，它为`MobXProviderContext`返回一个上下文钩子：
...
4.  最后，我们定义一个`useTodoStore`钩子，它从前面的钩子中获取`todoStore`，然后返回它：
...
现在，我们有一个通用的钩子，可以从 MobX 访问所有商店，还有一个特定的钩子可以访问`TodoStore`。如果需要，我们还可以在以后为其他商店定义更多挂钩。

# 将组件升级到挂钩

在创建一个钩子来访问我们的存储之后，我们可以使用它来代替使用`inject`高阶组件函数包装我们的组件。在接下来的部分中，我们将看到如何使用钩子升级各种组件。

# 使用应用程序组件的挂钩

我们将从升级`App`组件开始。可以逐步重构组件，以便使用钩子。我们不需要一次重构每个组件。
...
2.  然后，从我们的`hooks.js`文件中导入`useTodoStore`钩子：
...
4.  最后，使用我们的 Todo Store 钩子获取`todoStore`对象：

```
    const todoStore = useTodoStore()
```

正如你所看到的，我们的应用程序仍然像以前一样工作！然而，我们现在在`App`组件中使用了钩子，这使得代码更加简洁明了。

# 对 TodoList 组件使用挂钩

接下来，我们将升级我们的`TodoList`组件。此外，我们还将使用`useObserver`钩子，它将取代`observer`高阶组件。
...
2.  然后，从`mobx-react`导入`useObserver`钩子，从我们的`hooks.js`文件导入`useTodoStore`钩子：
...
4.  同样，我们使用 Todo Store 钩子来获取`todoStore`对象：

```
    const todoStore = useTodoStore()
```

5.  最后，我们用`useObserver`钩子包装返回的元素。当钩子中使用的状态更改时，将重新计算观察者钩子中的所有内容：
...
在我们的例子中，MobX 将检测通过`useObserver`钩子定义的观察者依赖于`todoStore.filteredTodos`，而`filteredTodos`依赖于`filter`和`todos`值。因此，每当`filter`值或`todos`数组更改时，列表将重新呈现。
...
5.  然后，使用 Todo Store 钩子获取`todoStore`对象：
...
7.  最后，我们用一个 Observer 钩子包装`return`语句，并在那里进行解构。这可确保 MobX 跟踪对`item`对象的更改，并且当对象的属性更改时，组件将相应地重新渲染：
...
如果`item.id`属性发生变化，我们必须将处理函数和`return`函数包装在一个`useObserver`钩子中，如下所示：
...
请注意，我们不能将处理程序函数和`return`语句包装在单独的观察者钩子中，因为这样处理程序函数只能在第一个观察者钩子的闭包中定义。这意味着我们将无法从第二个观察者钩子中访问处理程序函数。
...
4.  最后，使用 Todo Store 钩子获取`todoStore`对象：
...
5.  最后，我们用`useObserver`钩子包裹`style`对象。记住，当钩子中使用的状态发生变化时，观察者钩子中的所有内容都将重新计算：
...
除了提供全局存储以存储应用程序范围的状态外，MobX 还提供本地存储以存储本地状态。要创建本地商店，我们可以使用`useLocalStore`挂钩。

我们现在将在`AddTodo`组件中实现本地存储钩子：

1.  编辑`src/AddTodo.js`并从`mobx-react`导入`useLocalStore`钩子和`useObserver`钩子：
...
在这个本地存储中，我们可以定义状态值、计算值和操作。`useLocalStore`钩子将自动将值修饰为可观察值，将 getter 函数（前缀为`get`）修饰为计算值，将正常函数修饰为动作。
...
9.  最后，我们用一个`useObserver`钩子包装元素，以确保`input`字段值在变化时得到更新，并调整`disabled`和`value`道具：
...
现在，我们的`AddTodo`组件使用本地 MobX 存储来处理其输入值，并禁用/启用按钮。如您所见，使用 MobX，可以对本地和全局状态使用多个存储。困难的部分是决定如何以一种对给定应用程序有意义的方式拆分和分组您的商店。
...
在上一节中，我们学习了如何用 hook 替换现有 MobX 应用程序中的 MobX 高阶组件，例如`inject`和`observer`。现在，我们将学习如何将本地状态迁移到现有 MobX 应用程序中的挂钩。

可以通过以下三个步骤将现有 MobX 应用程序迁移到基于钩子的解决方案：

*   对简单的本地状态使用状态钩子
*   使用`useLocalState`钩子处理复杂的局部状态
*   在单独的 MobX 存储中保持全局状态

在本书的前几章中，我们已经学习了如何使用状态挂钩。状态钩子对于简单状态（例如复选框的当前状态）是有意义的。

在本章中，我们已经学习了如何使用`useLocalState`挂钩。我们可以对复杂的局部状态使用局部状态钩子，例如多个字段相互作用的复杂表单。然后，我们可以将多个状态和效果挂钩替换为单个局部状态挂钩以及计算值和操作。
...
*   如果我们想获得所有功能，MobX 需要一个包装器组件（`Provider`），以便将应用程序连接到应用商店（我们可以直接导入并使用 MobX 应用商店，但它会破坏服务器端渲染等功能）

如果状态更改很简单，并且只需要组件内的本地状态，则不应使用 MobX。在这种情况下，一个状态或减速器挂钩就足够了。有了 Reducer 和 State 挂钩，就不需要包装器组件来将我们的应用程序连接到应用商店。

灵活性是一件好事，但它也会导致我们的项目结构不好。然而，MobX 提供了一个名为`mobx-state-tree`的项目，它允许我们使我们的 MobX 应用程序更加结构化，并实施某种架构。更多信息可在以下 GitHub 存储库的项目页面上找到：[https://github.com/mobxjs/mobx-state-tree](https://github.com/mobxjs/mobx-state-tree) 。

# 总结

在本章中，我们首先了解了什么是 MobX，它由哪些元素组成，以及它们是如何协同工作的。然后，我们学习了如何在实践中使用 MobX 进行状态管理。我们还学习了如何通过使用`inject`和`observer`高阶组件连接 MobX 存储以反应组件。接下来，我们用钩子替换了高阶组件，这使我们的代码更加干净简洁。我们还学习了如何在 MobX 中使用本地存储钩子来处理复杂的本地状态。最后，我们学习了如何将现有的 MobX 应用程序迁移到 Hooks，并回顾了使用 MobX 的利弊。

这一章标志着这本书的结束。在这本书中，我们从使用挂钩的动机开始。我们了解到，React 应用程序中存在一些常见问题，如果没有挂钩，这些问题很难解决。然后，我们使用钩子创建了第一个组件，并将其与基于类组件的解决方案进行了比较。接下来，我们深入了解了各种钩子，从最普遍的状态钩子开始。我们还学习了如何使用钩子解决常见问题，例如条件钩子和循环中的钩子。

在深入了解状态钩子之后，我们使用钩子开发了一个小型博客应用程序。然后我们学习了 Reducer 钩子、Effect 钩子和 Context 钩子，以便能够在我们的应用程序中实现更多功能。接下来，我们学习了如何使用钩子高效地请求资源。此外，我们还学习了如何使用`React.memo`防止不必要的重新渲染，以及如何使用 React Suspense 实现延迟加载。然后，我们在我们的博客应用程序中实现了路由，并学习了挂钩如何使动态路由变得更容易。

我们还了解了社区提供的各种钩子，这些钩子使处理输入字段、各种数据结构、响应性设计和撤销/重做功能变得更加容易。此外，我们还了解了钩子的规则，如何创建自己的自定义钩子，以及钩子之间的交互是如何工作的。最后，我们学习了如何有效地从现有的基于类组件的应用程序迁移到基于钩子的解决方案。最后，我们学习了如何在 Redux 和 MobX 中使用 hook，以及如何将现有的 Redux 和 MobX 应用程序迁移到 hook 中。

现在我们已经深入了解了挂钩，我们已经准备好在应用程序中使用它们了！我们还学习了如何将现有项目迁移到钩子，现在就可以开始了。我希望您喜欢学习 React 钩子，并期待在应用程序中实现钩子！我确信使用钩子将使您的编码更加愉快，就像它们为我所做的那样。
...
7.  我们应该如何将现有的 MobX 应用程序迁移到 hook？
8.  使用 MobX 的优点是什么？
9.  使用 MobX 的缺点是什么？
10.  什么时候不应该使用 MobX？
