...
在上一章中，我们学习了如何使用 React 社区开发的各种钩子，以及在哪里可以找到更多钩子。我们学习了用钩子替换 React 生命周期方法、实用工具和数据管理钩子、用钩子替换响应性设计以及用钩子实现撤销/重做功能。最后，我们学会了在哪里找到其他钩子。

在本章中，我们将学习关于使用钩子的所有知识，以及在使用和开发我们自己的钩子时要注意的事项。钩子在调用顺序上有一定的限制。违反钩子规则可能会导致错误或意外行为，因此我们需要确保学习并执行规则。
...
钩子只能在***反应功能组件*或*自定义钩子*中调用。它们不能在类组件或常规 JavaScript 函数中使用。**
...
正如我们所看到的，钩子大部分是普通的 JavaScript 函数，只是它们依赖于在 React 函数组件中定义。当然，使用其他钩子的自定义钩子可以在 React 函数组件之外*定义*，但是当*使用*钩子时，我们总是需要确保在 React 函数组件内部调用它们。接下来，我们将学习关于钩子顺序的规则。

# 钩子顺序

仅调用功能组件或自定义挂钩的顶层/**开头**的挂钩**。**

不要在条件、循环或嵌套函数中调用钩子这样做会更改钩子的顺序，从而导致错误。我们已经了解到，更改钩子的顺序会导致状态在多个钩子之间混淆。

在[第 2 章](02.html)中*使用状态钩子*时，我们了解到我们无法执行以下操作：
...
![](Images/cc2657a7-8583-4cb4-8266-c13e22d2d7bd.png)
...
接下来，我们单击复选框以启用`firstName`字段。这样做改变了钩子的顺序，因为现在我们的钩子定义如下所示：

1.  `enableFirstName`
2.  `firstName`
3.  `lastName`

由于 React 完全依赖于钩子的顺序来管理它们的状态，`firstName`字段现在是第二个钩子，因此它从`lastName`字段获取状态：

![](Images/e8742fba-6847-40a0-9ce7-de2c5e7f560d.png)

Problem of changing the order of Hooks from Chapter 2, Using the State Hook

如果我们使用实例 2 中 React 中的真实`useState`钩子*我们可以定义条件钩子吗？*从[第 2 章](02.html)中*使用状态钩子*可以看到，当钩子的顺序发生变化时，React 会自动检测，并显示警告：

![](Images/4b1132d7-6f4a-4342-94e4-101b9bf33b36.png)

React printing a warning when detecting that the order of Hooks has changed

在开发模式下运行 React 时，如果渲染的钩子数多于上一个渲染中的钩子数，则它将另外崩溃，并显示未捕获不变冲突错误消息：

![](Images/217c1d37-366b-414a-a70d-64d9d5252f7e.png)

React crashing in development mode when the number of Hooks changed

正如我们所看到的，更改钩子的顺序或有条件地启用钩子是不可能的，因为 React 内部使用钩子的顺序来跟踪哪些数据属于哪个钩子。

# 钩子的名称

有一种约定，钩子函数的前缀应该始终是`use`，然后是以大写字母开头的钩子名称；例如：`useState`、`useEffect`和`useResource`。这很重要，因为否则我们将不知道哪些 JavaScript 函数是钩子，哪些不是。特别是在强制执行钩子规则时，我们需要知道哪些函数是钩子，这样我们就可以确保它们没有被条件调用或循环调用。

正如我们所看到的，命名约定在技术上并不是必需的，但它们使开发人员的生活更加轻松。了解正常函数和钩子之间的差异可以很容易地自动执行钩子规则。在下一节中，我们将学习如何使用`eslint`工具自动执行规则。

# 执行挂钩规则

如果我们坚持使用`use`作为钩子函数前缀的约定，我们可以自动执行另外两个规则：

*   仅从 React 函数组件或自定义挂钩调用挂钩
*   仅在顶层调用挂钩（不在循环、条件或嵌套函数内部）

为了自动执行规则，React 提供了一个名为`eslint-plugin-react-hooks`的`eslint`插件，该插件将自动检测何时使用了钩子，并将确保规则不会被破坏。ESLint 是一个 linter，它是一个分析源代码并发现诸如风格错误、潜在 bug 和编程错误等问题的工具。
...
![](Images/647045f4-5f7a-4179-a026-a2f7a347dab1.png)

Executing ESLint with the react-hooks plugin

我们现在将试图打破钩的规则；例如，通过编辑`src/user/Login.js`并使第二个输入钩子有条件：
...
![](Images/53184548-1539-43e2-8ab0-4bd38f34e9c6.png)

Executing ESLint after breaking the rules of Hooks

正如我们所看到的，`eslint`通过强迫我们遵守钩子的规则来帮助我们。当我们违反任何规则时，linter 将抛出一个错误，并在效果挂钩缺少依赖项时显示警告。聆听`eslint`将有助于我们避免错误和意外行为，因此我们永远不能忽视它的错误或警告。
...
除了强制执行钩子的规则外，我们还检查 Effect 钩子中使用的所有变量是否都传递给它的依赖项数组。这个*彻底依赖*规则确保每当效果挂钩中使用的内容发生变化（函数、值等）时，挂钩将再次触发。
...
![](Images/aec44b71-f218-47d5-bcde-25306ede9272.png)
...
在本章中，我们首先了解了钩子的两个规则：我们应该只从 React 函数组件调用钩子，并且我们需要确保钩子的顺序保持不变。此外，我们还了解了钩子的命名约定，它们应该始终以`use`前缀开头。然后，我们学习了如何使用`eslint`强制执行挂钩规则。最后，我们学习了`useEffect`依赖项，以及如何使用`eslint`自动修复缺失的依赖项。

为了避免 bug 和意外行为，了解钩子的规则并加以实施是非常重要的。这些规则在创建我们自己的钩子时尤其重要。现在我们已经很好地掌握了钩子的工作原理，包括它们的规则和约定，在下一章中，我们将学习如何创建自己的钩子！
