...
我们正在从Falcor的模型`(await falcorModel.getValue('login.username'))`中获取用户名。我们正在获取用户的角色`(await falcorModel.getValue('login.role'))`。然后我们将所有已知变量从后端保存到`localStoragewith`：
...
完成后，我们将返回一个`INVALID`信息给Falcor的模型：
