...
*   Webpack 4 零配置
*   将 React 添加到网页 4
*   使用 React 添加Webpack开发服务器和 Sass、手写笔或 LESSCS
*   Webpack 4 优化-拆分捆绑包
...
11.  如果要使用较少的 CSS，请对Webpack配置进行必要的更改，然后使用此文件：
...
6.  我们使用提取文本插件只是为了生产。对于任何其他环境，我们都像以前一样直接使用`style-loader`、`css-loader`和`sass-loader`。这就是为什么我喜欢将Webpack配置拆分成更小的文件，正如您所看到的，有些文件可能很大，因此这有助于我们更有序地组织。如果您使用`npm run start-production`启动生产模式，您将看到以下 CSS：
...
![](assets/6f0cc143-f0e0-47e1-a109-6c9aefb9ef1f.png)
...
![](assets/4475abf7-6a7f-4fb8-9c4a-3d7170092db2.png)
