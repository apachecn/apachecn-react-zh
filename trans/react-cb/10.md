...
6.  我们使用提取文本插件只是为了生产。对于任何其他环境，我们都像以前一样直接使用`style-loader`、`css-loader`和`sass-loader`。这就是为什么我喜欢将网页包配置拆分成更小的文件，正如您所看到的，有些文件可能很大，因此这有助于我们更有序地组织。如果您使用`npm run start-production`启动生产模式，您将看到以下 CSS：
