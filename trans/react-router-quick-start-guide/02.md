...
`staticContext`属性是在服务器端呈现应用程序时设置的，在客户端路由（即使用`<BrowserRouter>`接口）中不可用（如中，设置为`undefined`。接下来的章节将介绍应用程序的服务器端呈现。

# 历史

React 路由依赖于`history`包。`history`是一个 JavaScript 库，用于维护任何 JavaScript 应用程序中的会话。考虑下面的历史文献的引用 https://github.com/ReactTraining/history ：
